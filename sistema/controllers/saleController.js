...
export default {
    ...
    list: async (req,res,next) => {
        try {
            ...
            const reg = await models.Sale.find({$or:[{'proofNumber': new RegExp(keyword,'i')},{'proofSeries': new RegExp(keyword,'i')}]})
            .populate('user',{name:1})
            .populate('person',{name:1})
            .sort(
                {
    ...
    activate: async (req,res,next) => {
        try {
            const reg = await models.Sale.findByIdAndUpdate({_id: req.body._id},{
                status: 1
            })
            // Update stock
